  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="font-display text-[14px] font-black italic tracking-tight">
          Start
        </span>
      </button>
        ...
        <div className="absolute bottom-10 left-0 w-64 bg-[#c0c0c0] p-0.5 retro-bevel-out window-shadow z-1001">
          <div className="flex">
            <div className="w-8 bg-[#808080] flex items-end justify-center py-4">
              <span className="text-white font-bold -rotate-90 whitespace-nowrap text-lg tracking-widest font-display">
                SURF_PORT OS
              </span>
            </div>
            <div className="grow p-1">
              {menuItems.map((item, i) =>
                "divider" in item ? (
                  <div
                    key={i}
                    className="h-px bg-[#808080] my-1 shadow-[0_1px_0_0_#fff]"
                  ></div>
                ) : (
                  <button
                    key={item.id}
